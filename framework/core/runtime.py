...
NAMES = ("scheduler", "kapi", "io", "memory", "virt-ept", "virt-io", "virt-virtio", "virt-vtd")
...
def read_json(path):
    ...
    return json.loads(Path(path).read_text(), object_pairs_hook=pairs,
                      parse_constant=lambda v: (_ for _ in ()).throw(LabError("invalid JSON number: " + v)))
...
def command(argv, *, cwd=None, timeout=120, input=None, capture=True, env=None):
    ...
    if result.returncode:
        ...
        raise LabError("command failed (%s): %s\n%s" % (result.returncode, argv[0], detail))
...
ENVELOPE = {"experiment", "sequence", "source", "kind", "timestamp_ns", "clock_domain", "context", "quality", "data"}
LIFECYCLE = {"capture_started", "collector_ready", "workload_started", "workload_finished",
             "collector_finished", "capture_finished", "capture_failed"}
    ...
    def walk(value, path):
        if isinstance(value, dict):
            for key, child in value.items():
                loc = path + "." + key
                if ("truncated" in key and child not in (False, "0", None)) or (key == "sample_status" and child not in ("complete", "ok")):
...
def record(experiment, collector, mechanism, domain, kind, timestamp, data, *, hook=None, context=None):
    ctx = dict.fromkeys(CONTEXT)
    supplied = context or {}
    for key in CONTEXT:
        value = supplied.get(key)
        ctx[key] = str(value) if key in ("phase", "operation") and value is not None else value
    payload = normalize(data)
    return {"experiment": experiment, "sequence": 0,
            "source": {"collector": collector, "mechanism": mechanism, "domain": domain, "hook": hook},
            "kind": kind, "timestamp_ns": str(timestamp), "clock_domain": domain + ":monotonic",
            "context": ctx, "quality": quality(payload), "data": payload}
...
def observation(experiment, collector, domain, raw, acquired_ns):
    ...
    context["operation"] = info.get("operation_id", info.get("request_id", context.get("operation")))
    reserved = {"experiment", "kind", "source", "seq", "time_ns", "clock", "context"}
    ...
    if extra:
        data["subject"] = extra
    return record(experiment, collector, "workload" if collector == "workload" else "ebpf", domain,
                  kind, raw.get("time_ns", acquired_ns), data,
                  hook=info.get("probe", info.get("hook")), context=context)


TRACE = re.compile(r"^\s*(.+?)-(\d+)\s+(?:\(\s*\d+\)\s+)?\[(\d+)\]\s+\S+\s+(\d+\.\d+):\s+(.*)$")
...
def trace_record(experiment, domain, line):
    ...
    syscall_io = re.match(r"sys_(read|write)\(fd:\s*(\S+),\s*buf:\s*(\S+).*?,\s*count:\s*(\S+)\)", detail)
    if syscall_io:
        name, fd, buf, count = syscall_io.groups()
        return record(experiment, "tracefs", "tracefs", domain, "sys_enter_" + name,
    ...
    if syscall:
        ...
        fields = key_values(syscall[2]) if syscall[2] is not None else {"result": syscall[3]}
    else:
    ...
    return record(experiment, "tracefs", "tracefs", domain, kind,
                  int(Decimal(seconds) * 1_000_000_000), {"fields": fields},
                  hook=kind, context={"tid": int(tid), "cpu": int(cpu), "comm": comm.strip()})
...
def key_values(payload):
    ...
    for i, field in enumerate(matches):
        ...
        fields[key] = payload[field.end():matches[i + 1].start() if i + 1 < len(matches) else None].strip(" ,")
    return fields
            ...
            try:
                def pairs(items):
                    ...
                    for key, value in items:
                        ...
                        result[key] = value
                    return result
                yield json.loads(line, object_pairs_hook=pairs, parse_constant=lambda x: (_ for _ in ()).throw(ValueError(x)))
            except ValueError as exc:
                raise LabError("invalid JSON at %s:%d: %s" % (path, index, exc)) from exc
...
def check_type(value, spec, path="data", types=None):
    ...
    if typ == "object":
        ...
        if extra or missing:
            raise LabError("%s: unknown %s, missing %s" % (path, sorted(extra), sorted(missing)))
        for key, child in value.items():
...
def validate_record(event, name, catalog, sequence):
    if set(event) != ENVELOPE or event["experiment"] != name or event["sequence"] != sequence:
        raise LabError("invalid envelope or sequence at record %d" % sequence)
    if not isinstance(event["timestamp_ns"], str) or not re.fullmatch(r"0|[1-9][0-9]*", event["timestamp_ns"]):
    ...
    if source["domain"] not in ("host", "guest") or event["clock_domain"] != source["domain"] + ":monotonic":
        raise LabError("invalid clock domain")
    if source["mechanism"] not in ("framework", "ebpf", "tracefs", "workload", "module", "sysfs"):
    ...
    for key in ("cpu", "pid", "tid"):
        if ctx[key] is not None and (type(ctx[key]) is not int or not 0 <= ctx[key] < 2**32):
            raise LabError("invalid context " + key)
    for key in ("comm", "phase", "operation"):
        if ctx[key] is not None and not isinstance(ctx[key], str):
            raise LabError("invalid context " + key)
    q = event["quality"]
    if set(q) != {"status", "reasons"} or q["status"] not in ("complete", "partial", "unavailable") or not isinstance(q["reasons"], list):
...
def validate(path, name, *, evidence=True):
    ...
    if len(events) < 3 or events[0]["kind"] != "capture_started" or events[-1]["kind"] != "capture_finished":
...
class Capture:
    ...
    def lifecycle(self, kind, data):
        self.events.append(record(self.name, "controller", "framework", self.domain,
                                  kind, time.monotonic_ns(), data))

    def save(self):
        for seq, event in enumerate(self.events, 1):
            event["sequence"] = seq
        atomic_bytes(self.path, b"".join((json.dumps(e, separators=(",", ":"), allow_nan=False) + "\n").encode() for e in self.events))
...
def validate_evidence(name, events):
    ...
    require(kinds["capture_started"] == kinds["capture_finished"] == 1, "exactly one capture lifetime required")
    require(kinds["workload_started"] > 0 and kinds["workload_finished"] > 0, "missing workload boundaries")
    require(events[-1]["data"] == {"validated": True, "restored": True}, "unsuccessful final status")
    ready = Counter(e["data"]["collector"] for e in events if e["kind"] == "collector_ready")
    done = Counter(e["data"]["collector"] for e in events if e["kind"] == "collector_finished")
    require(ready and ready == done, "collector readiness/completion mismatch")
    for event in events:
        if event["kind"] == "collector_finished":
            require(event["data"]["dropped"] == event["data"]["failures"] == "0", "collector reported evidence loss")
        if event["quality"]["status"] != "complete":
            require(cfg["allow_partial"] and event["quality"]["status"] == "partial", "undeclared partial observation")
        if event["kind"] == "collector_summary":
            state = event["data"].get("state", {})
            require(state.get("ringbuf_dropped") == "0" and state.get("short_records") == "0", "observer summary reports loss")

    def selected(kind, mechanism=None):
        return [event for event in events if event["kind"] == kind and
                (mechanism is None or event["source"]["mechanism"] == mechanism)]

    if name == "scheduler":
        for event in selected("enqueue_entity"):
            runqueue = event["data"]["state"]["runqueue"]
            require(int(runqueue["node_count"]) == len(runqueue["nodes"]), "scheduler node count differs from payload")
        require(selected("sched_process_fork", "tracefs") and selected("sched_switch", "tracefs"), "missing scheduler lifecycle context")
    elif name == "memory":
        phases = selected("phase", "workload")
        snapshots = selected("memory_snapshot", "ebpf")
        require([event["context"]["phase"] for event in phases] == [event["context"]["phase"] for event in snapshots], "memory phases and snapshots do not pair")
        ...
        require(any(event["data"].get("domain") == "module" for event in selected("ready", "module")), "module never reached ready")
    elif name == "virt-ept":
        for kind, field, count in (("control", "control", 5), ("memslot", "memslot", 6)):
            begins = selected(kind + "_begin", "ebpf")
            ends = selected(kind + "_end", "ebpf")
            require(len(begins) == len(ends) == count, "wrong EPT " + kind + " count")
            require({event["data"][field]["operation_id"] for event in begins} ==
                    {event["data"][field]["operation_id"] for event in ends}, "unpaired EPT " + kind)
            require(all(event["data"][field]["result"] == "0" for event in ends), "failed EPT " + kind)
    elif name == "virt-virtio":
        begins = selected("queue_backend_begin", "ebpf")
        ends = selected("queue_backend_end", "ebpf")
        require(len(begins) == len(ends) == 3, "expected three virtqueue backend operations")
        require(len(selected("ioeventfd_kick", "ebpf")) == len(selected("irqfd_signal", "ebpf")) == 1, "missing eventfd handoff")
        require([event["data"]["state"]["used"]["idx"] for event in ends] == ["1", "5", "6"], "virtqueue completion indices differ")
    elif name == "virt-vtd":
        lifecycle = [event["kind"] for event in events if event["source"]["mechanism"] == "sysfs"]
        require(lifecycle == ["host_owns_device", "vfio_bound", "qemu_attached", "guest_visible", "host_reclaims_device"], "incomplete PCI assignment lifetime")
        for event in selected("guest_ixgbe_run_loopback_exit", "ebpf"):
            require(event["data"]["event_info"]["result"] == "0", "guest loopback failed")
        require(len(selected("guest_irq_handler_entry")) == len(selected("guest_irq_handler_exit")), "unpaired guest IRQ boundaries")
...
class Process:
    def __init__(self, argv, *, cwd, stdout, stderr, env=None, control=False):
        ...
        try:
            ...
            self.child = subprocess.Popen([str(x) for x in argv], cwd=cwd, env=child_env,
                                          stdin=subprocess.PIPE, stdout=self.out, stderr=self.err, start_new_session=True,
...
class Trace:
    def __init__(self, name, runtime, buffer_kib):
        ...
        if self.path.exists():
            raise LabError("stale trace instance requires inspection: " + str(self.path))
    ...
    def enable(self, events, required=True):
        catalog = read_json(ROOT / "experiments" / self.path.name.removeprefix("lab-") / "payloads.json")
        formats = self.runtime / "trace-formats"
        formats.mkdir(exist_ok=True)
        for event in events:
            if "tracefs:" + event.split("/")[1] not in catalog:
                if required:
                    raise LabError("required tracepoint has no reviewed payload contract: " + event)
            ...
            (formats / event.replace("/", "__")).write_text((path / "format").read_text())
        ...
        for path in sorted((self.path / "per_cpu").glob("cpu*/stats")):
            ...
            for key in ("overrun", "commit overrun", "dropped events"):
                ...
                if int(values[key].strip()):
                    raise LabError("trace data loss: %s %s=%s" % (path.parent.name, key, values[key]))
        if not stats:
...
def module_record(line):
    ...
    for token in shlex.split(match[2]):
        ...
        data[key] = value
    return record("kapi", "module", "module", "guest", data["action"],
                  int(Decimal(match[1]) * 1_000_000_000), data, hook="kapi:printk",
                  context={"phase": data["phase"], "cpu": int(data["cpu"]) if "cpu" in data else None,
...
def stopped(process, timeout):
    ...
    while time.monotonic() < end:
        ...
        if re.search(r"^State:\s+T", path.read_text(), re.M):
            return
        time.sleep(0.02)
...
class Session:
    def __init__(self, name, domain):
        ...
        if shared:
            ...
            self._scratch = tempfile.TemporaryDirectory(prefix="linuxmaxxing-%s-" % name, dir=scratch_parent)
            self.runtime = Path(self._scratch.name)
        self.cwd = (Path("/mnt/host") / self.cfg["source"]
                    if domain == "guest" else ROOT / "build/tree" / self.cfg["source"])
    ...
    def process(self, argv, label, control=False, cwd=None):
        proc = Process(argv, cwd=cwd or self.cwd, stdout=self.runtime / (label + ".ndjson"),
                       stderr=None, control=control)
        self.stack.callback(proc.close)
        self.stack.callback(proc.stop)
        return proc

    def observer(self, argv=None, label="observer"):
        proc = self.process(argv or [self.cwd / self.cfg["observer"]], label, control=True)
        ...
        if self.name == "scheduler":
            ...
            for event in self.trace.enabled:
                if event == "sched/sched_switch":
                    self.trace.filter(event, 'prev_comm ~ "sched*" || next_comm ~ "sched*"')
                elif event == "sched/sched_process_fork":
                    self.trace.filter(event, 'parent_comm ~ "sched*" || child_comm ~ "sched*"')
                else:
    ...
    def collect(self):
        for proc, label in self.observers:
            ...
            for raw in ndjson(self.runtime / (label + ".ndjson")):
                self.capture.events.append(observation(self.name, label, self.domain, raw, self.acquired))
            self.capture.lifecycle("collector_finished", {"collector": label, "dropped": 0, "failures": 0})
        if self.trace:
            self.trace.stop()
            with (self.runtime / "trace.txt").open() as stream:
                for line in stream:
                    try:
                        ...
                        raise LabError("trace decode failed: %s | %s" % (exc, line.strip())) from exc
                    if event:
                        self.capture.events.append(event)
            self.capture.lifecycle("collector_finished", {"collector": "tracefs", "dropped": 0, "failures": 0})
        if self.name in ("memory", "io"):
            for raw in ndjson(self.runtime / "workload.ndjson"):
                self.capture.events.append(observation(self.name, "workload", self.domain, raw, self.acquired))

    def io_prepare(self):
        self.module("qedu_trace")
        self.trace_start()
        devices = [p for p in Path("/sys/bus/pci/devices").iterdir()
                   if (p / "vendor").read_text().strip() == "0x1234" and (p / "device").read_text().strip() == "0x11e8"]
        if len(devices) != 1:
        ...
        for event in ("irq/irq_handler_entry", "irq/irq_handler_exit", "irq_vectors/vector_alloc", "irq_vectors/vector_config"):
        ...
        symbols = {fields[2]: fields[0] for line in Path("/proc/kallsyms").read_text().splitlines()
        ...
        for event in ("workqueue/workqueue_activate_work", "workqueue/workqueue_execute_start", "workqueue/workqueue_execute_end"):
    ...
    def kapi(self):
        ...
        self.capture.lifecycle("workload_started", {"command": ["/sbin/insmod", "kapi.ko"]})
        ...
        self.capture.lifecycle("collector_finished", {"collector": "module", "dropped": 0, "failures": 0})

    def execute(self):
        if os.geteuid() != 0:
            raise LabError("experiment worker requires root in its execution environment")
        facts = read_json(ROOT / "build/environment.json")
        self.capture.lifecycle("capture_started", {"environment": facts, "source_digest": read_json(ROOT / "source-provenance.json")["digest"],
                                                  "enabled": [], "skipped": []})
        try:
            ...
            self.stack.close()
            self.capture.lifecycle("capture_finished", {"validated": True, "restored": True})
            self.capture.save()
            publish(self.capture.path, ROOT / "captures" / self.name / "events.ndjson", self.name)
        except BaseException as exc:
            try:
                self.stack.close()
            except BaseException as cleanup:
                exc = LabError("%s; cleanup failed: %s" % (exc, cleanup))
            self.capture.lifecycle("capture_failed", {"error": str(exc) or type(exc).__name__})
            self.capture.save()
            raise LabError(str(exc)) from exc
...
def prepare_image():
    ...
    if temporary.exists():
        temporary.unlink()
    command(["qemu-img", "convert", "-O", "qcow2", ROOT / "build/images/ubuntu.qcow2", temporary], timeout=600)
    customize = ["sudo", "-n", "virt-customize", "-a", temporary, "--install", expected["packages"]]
    # The first libguestfs invocation on a fresh CloudLab node can race its
...
def run(name, console=False):
    ...
    with tempfile.TemporaryDirectory(prefix="linuxmaxxing-guest-", dir=build) as scratch_name:
        ...
        argv = ["qemu-system-x86_64", "-accel", "tcg", "-m", str(cfg["guest_memory_mib"]),
                "-smp", str(cfg["guest_cpus"]), "-kernel", str(ROOT / "build/kernel/arch/x86/boot/bzImage"),
                "-append", "root=/dev/vda1 console=ttyS0 rootwait rw loglevel=4 init=/bin/sh",
                "-drive", "file=%s,if=virtio,format=qcow2" % overlay,
                "-drive", "file=%s,if=virtio,format=raw" % swap,
        ...
        with ExitStack() as stack:
            ...
            script = ("mount -t proc proc /proc; mount -t sysfs sysfs /sys; "
                      ...
                      "python3 /mnt/lab/framework/cli.py --guest run " + name + "; sync; poweroff -f\n")
