"""CloudLab provisioning, source transport, and remote execution."""
import hashlib
import io
import json
import os
...
from framework.core.runtime import LabError, ROOT, atomic_json, command, digest, manifest, read_json
...
def doctor(check_tools=True):
    ...
    if platform.system() != "Linux" or platform.machine() != expected["architecture"]:
        raise LabError("execution requires an x86-64 Linux CloudLab host")
    release = dict(line.split("=", 1) for line in Path("/etc/os-release").read_text().splitlines() if "=" in line)
    if release.get("ID", "").strip('"') != expected["os"] or release.get("VERSION_ID", "").strip('"') != expected["release"]:
        raise LabError("use the Ubuntu 24.04 CloudLab image")
    command(["sudo", "-n", "true"])
    if check_tools:
        missing = [tool for tool in ("gcc", "clang", "make", "bpftool", "qemu-system-x86_64", "qemu-img", "cloud-localds", "virt-customize") if not shutil.which(tool)]
        if missing:
            raise LabError("missing host tools: %s; run ./lab.sh setup" % ", ".join(missing))
    return {"kernel": platform.release(), "architecture": platform.machine(), "os": "ubuntu-24.04"}
...
def setup():
    ...
    command(["sudo", "-n", "env", "DEBIAN_FRONTEND=noninteractive", "apt-get", "install", "-y"] + cfg["packages"], timeout=1800, capture=False)
    secure(cfg["security"])
    ...
    if not base.exists() or digest(base) != cfg["image_sha256"]:
        ...
        command(["curl", "--fail", "--location", "--retry", "3", "--output", temporary, cfg["image_url"]], timeout=1800, capture=False)
    ...
    if not kernel.exists():
        ...
        command(["git", "-C", kernel, "remote", "add", "origin", cfg["kernel_repository"]])
    else:
        remotes = command(["git", "-C", kernel, "remote"]).decode().split()
        if "origin" not in remotes:
            command(["git", "-C", kernel, "remote", "add", "origin", cfg["kernel_repository"]])
    if command(["git", "-C", kernel, "remote", "get-url", "origin"]).decode().strip() != cfg["kernel_repository"]:
        command(["git", "-C", kernel, "remote", "set-url", "origin", cfg["kernel_repository"]])
    command(["git", "-C", kernel, "fetch", "--depth=1", "origin", cfg["kernel_revision"]], timeout=1800, capture=False)
    ...
    packages = command(["dpkg-query", "-W", "-f=${Package}=${Version}\n"] + cfg["packages"]).decode().splitlines()
    facts.update(packages=packages, image_sha256=cfg["image_sha256"], kernel_revision=actual,
                 qemu=command(["qemu-system-x86_64", "--version"]).decode().splitlines()[0])
    atomic_json(ROOT / "build/environment.json", facts)
    print("CloudLab dependencies and pinned sources ready. Builds occur on demand.")
...
def secure(cfg):
    ...
    command(["sudo", "-n", "ufw", "allow", port + "/tcp", "comment", "CloudLab SSH"], capture=False)
    command(["sudo", "-n", "ufw", "--force", "enable"], capture=False)
    if cfg.get("fail2ban"):
        jail = "[sshd]\n" "enabled = true\n" "port = %s\n" "maxretry = 5\n" "findtime = 10m\n" "bantime = 1h\n" % port
        command(["sudo", "-n", "tee", "/etc/fail2ban/jail.d/linuxmaxxing-sshd.local"], input=jail.encode())
        command(["sudo", "-n", "systemctl", "enable", "--now", "fail2ban"], timeout=120, capture=False)
    if cfg.get("unattended_upgrades"):
        command(["sudo", "-n", "systemctl", "enable", "--now", "apt-daily.timer", "apt-daily-upgrade.timer"], timeout=120, capture=False)
...
def build(name):
    ...
    if cfg["environment"] == "host":
        ...
        if not Path("/dev/kvm").exists() or not Path("/sys/kernel/btf/vmlinux").exists():
            raise LabError("KVM or host kernel BTF is unavailable")
    if cfg["environment"] == "guest":
        env = dict(os.environ, LAB_KERNEL_SOURCE=str(ROOT / "build/linux"), LAB_KERNEL_BUILD=str(ROOT / "build/kernel"))
        command(["bash", ROOT / "infra/build-kernel.sh"], timeout=10800, capture=False, env=env)
    source = ROOT / cfg["source"]
    target = ROOT / "build/tree" / cfg["source"]
    target.mkdir(parents=True, exist_ok=True)
    identity = {"kernel_btf": digest(ROOT / "build/kernel/vmlinux") if cfg["environment"] == "guest" else digest("/sys/kernel/btf/vmlinux")}
    ...
    for path in source.iterdir():
        if path.is_file() and (path.suffix in (".c", ".h", ".S") or path.name == "Makefile"):
    ...
    for path in (ROOT / "common").iterdir():
        if path.suffix in (".c", ".h"):
            ...
            if not destination.exists() or digest(path) != digest(destination):
                shutil.copy2(path, destination)
    command(["make", "-C", target, "all", "CC=gcc", "ARCH=x86_64",
             "KDIR=" + str(ROOT / "build/kernel"), "KERNEL_BUILD=" + str(ROOT / "build/kernel"),
             "SHARED_DIR=" + str(common)], timeout=1800, capture=False)
    return target
...
def prepare_vtd():
    ...
    if groups.exists() and any(groups.iterdir()):
        ...
        print("VT-d candidate %s; management interface %s remains protected" % (pci.name, management.name))
        return
    ...
    command(["sudo", "-n", "tee", "/etc/default/grub.d/90-linuxmaxxing.cfg"], input=content)
    command(["sudo", "-n", "update-grub"], timeout=120, capture=False)
    print("IOMMU boot configuration installed. Reboot the node, then repeat ./lab.sh prepare-vtd.")
...
with (root / '.lock').open('a') as mutex:
    try:
        ...
        sys.exit('CloudLab workspace is busy')
...
class Remote:
    def __init__(self, config=None):
        path = Path(config) if config else ROOT / "cloudlab.json"
        if not path.exists():
            raise LabError("missing cloudlab.json; copy infra/cloudlab.example.json and set the target")
        cfg = read_json(path)
        if set(cfg) != {"target", "workspace"}:
            raise LabError("cloudlab.json accepts only target and workspace; put key/port settings in SSH config")
        ...
        with tarfile.open(fileobj=buf, mode="w") as tar:
            ...
            for directory in ("framework", "infra", "experiments", "common"):
                for path in (ROOT / directory).rglob("*"):
                    ...
                    if any(p in ("__pycache__", "_captures", "build", "_exps") for p in rel.parts):
                        continue
                    if path.is_file() and not path.is_symlink() and (path.suffix in (".py", ".json", ".c", ".h", ".S", ".sh", ".md") or path.name == "Makefile"):
                        paths.append(path)
    ...
    def execute(self, action, name=""):
        argv = self.ssh + [shlex.join(["python3", "-c", BOOTSTRAP, self.workspace, action, name])]
    ...
    def fetch(self, name):
        ...
        return command(self.ssh + [shlex.join(["python3", "-c", script, path])], timeout=600)
