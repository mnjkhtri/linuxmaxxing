 ...
 * The framework owns the canonical envelope; this observer emits only payloads.
...
int main(void)
{
	...
	lab_control("LX_READY experiment=virt-ept observer=ept\n");
...
out:
	...
	if (err)
		return 1;
	lab_control("LX_DONE experiment=virt-ept observer=ept records=%u\n", record_count);
	return 0;
}
