...
int BPF_UPROBE(observe_vmm_main)
{
	...
	lab_map_update(&experiment_tasks, &key, &active, BPF_ANY);
	return 0;
}
...
static __always_inline __u64 next_operation_id(__u64 key)
{
	...
	if (last)
		next = *last + 1;
	lab_map_update(&operation_counters, &key, &next, BPF_ANY);
	return next;
}
...
int observe_ioctl_enter(struct trace_event_raw_sys_enter *ctx)
{
	...
	lab_map_update(&ioctl_counters, &key, &next, BPF_ANY);
	lab_map_update(&active_ioctl, &key, &args, BPF_ANY);
...
int observe_madvise_enter(struct trace_event_raw_sys_enter *ctx)
{
	...
	lab_map_update(&madvise_counters, &key, &next, BPF_ANY);
	lab_map_update(&active_madvise, &key, &args, BPF_ANY);
...
int observe_mmap_enter(struct trace_event_raw_sys_enter *ctx)
{
	...
	lab_map_update(&mmap_counters, &key, &next, BPF_ANY);
	lab_map_update(&active_mmap, &key, &args, BPF_ANY);
...
static __always_inline int snapshot_vcpu_mm(struct kvm_vcpu *vcpu, enum ept_event_type event_type)
{
	...
	if (!event || !vcpu)
		return 0;
	lab_map_update(&current_vcpu, &key, &vcpu, BPF_ANY);
	sample_vcpu_state(event, vcpu);
	emit_event(event);
	return 0;
}
...
int BPF_UPROBE(observe_control_begin, void *context, __u8 command)
{
	...
	lab_map_update(&active_control, &key, &args, BPF_ANY);
...
int BPF_UPROBE(observe_memslot_begin, int vm_fd, __u32 slot, __u64 guest_phys_addr, void *memory, __u64 size, __u32 flags)
{
	...
	lab_map_update(&active_memslot, &key, &args, BPF_ANY);
