...
int main(int argc, char **argv)
{
    ...
    /* Detach before draining so a busy host cannot keep producing indefinitely. */
    for (unsigned int index = 0; index < link_count; index++) {
        bpf_link__destroy(links[index]);
        links[index] = NULL;
    }
    stop_requested = 1;
    if (lab_poll(ring_buffer, &stop_requested) < 0)
        goto cleanup;
    if (bpf_map_lookup_elem(bpf_map__fd(skeleton->maps.dropped_events), &map_key, &dropped))
        goto cleanup;
    if (emit_summary(dropped))
        goto cleanup;
    status = (poll_result < 0 && poll_result != -EINTR) || dropped || short_record_count;
    if (lab_health(dropped, skeleton->bss->lab_failures + short_record_count))
        status = 1;
