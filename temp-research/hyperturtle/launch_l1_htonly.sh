...
if [ "$PASSTHROUGH_DEVS" -eq "0" ]; then
    ...
    if [ $is_virtualized -eq "0" ]; then
        ...
        for ((i = 1; i < $N_PASSTHROUGH_DEVICES; i++)); do
            ...
            NET="$NET -device virtio-net-pci,netdev=net$i,disable-legacy=on,disable-modern=off,bus=rp$i,mrg_rxbuf=on,rx_queue_size=1024,tx_queue_size=1024,multifunction=on -netdev tap,id=net$i,ifname=tap$i,script=no,downscript=no,br=br0,poll-us=1000"
        done
        ...
        NET="$NET -device virtio-net-pci,netdev=net20,mrg_rxbuf=on,rx_queue_size=1024,tx_queue_size=1024,disable-legacy=on,disable-modern=off,multifunction=on -netdev tap,id=net20,ifname=tap20,script=no,downscript=no,br=br0,poll-us=1000,sndbuf=10000"
