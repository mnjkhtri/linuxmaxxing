...
static void show_idle_thread(void)
{
	...
	pr_info("KAPI_EVT domain=core phase=tasks action=task tgid=%d pid=%d ppid=0 cpu=0 task=%px stack=%px mm=%px type=idle comm=\"%s\"\n",
...
static int show_all_threads(void)
{
	...
	pr_info("KAPI_EVT domain=core phase=tasks action=begin scope=all_threads\n");
	...
	for_each_process_thread(p, t)
	{
		...
		pr_info("KAPI_EVT domain=core phase=tasks action=task tgid=%d pid=%d ppid=%d cpu=%d task=%px stack=%px mm=%px type=%s comm=\"%s\"\n",
				t->tgid,
				t->pid,
				task_ppid_nr(t),
				task_cpu(t),
...
static int __init core_kapi_init(void)
{
	int total;

	pr_info("KAPI_EVT domain=core phase=lifecycle action=attached module=%s\n", KBUILD_MODNAME);
	total = show_all_threads();
	pr_info("KAPI_EVT domain=core phase=tasks action=end total=%d\n", total);

	return 0;
}

static void __exit core_kapi_exit(void)
{
	pr_info("KAPI_EVT domain=core phase=lifecycle action=detached module=%s\n", KBUILD_MODNAME);
}
...
#define CUSTOM_CACHE_NAME "kapi_mm_cache"
...
static int __init mm_kapi_init(void);
...
{
	pr_info("KAPI_EVT domain=mm phase=layout action=address id=%s addr=0x%016lx\n",
			name, addr);
}
			 ...
			 const char *kind, const char *source)
{
	if (end <= start)
	{
		pr_warn("KAPI_EVT domain=mm phase=layout action=region id=%s start=0x%016lx end=0x%016lx kind=%s source=%s valid=no\n",
				name, start, end, kind, source);
		return;
	}

	pr_info("KAPI_EVT domain=mm phase=layout action=region id=%s start=0x%016lx end=0x%016lx bytes=%lu kind=%s source=%s valid=yes\n",
...
#define PRINT_ORDERED(name, start, end, kind, source) \
	...
	pr_info("KAPI_EVT domain=mm phase=layout action=begin\n");
	pr_info("KAPI_EVT domain=mm phase=layout action=constants page_size=%lu page_shift=%u page_mask=0x%016lx bits_per_long=%u ram_bytes=%lu paging_levels=%u\n",
	...
	print_addr("mm_kapi_init", (unsigned long)mm_kapi_init);
	...
	pr_info("KAPI_EVT domain=mm phase=layout action=end\n");
...
static void sample_memory(const char *name, const void *addr, size_t len)
{
	...
	pr_info("KAPI_EVT domain=mm phase=memory action=sample id=%s bytes=%d data=%*phN\n",
...
static void show_pages(const char *name, const void *addr, size_t len)
{
	...
	for (i = 0; i < pages; i++)
	{
		...
		pr_info("KAPI_EVT domain=mm phase=mapping action=page id=%s index=%u kva=%px pa=%pa pfn=%lu\n",
	...
	pr_info("KAPI_EVT domain=mm phase=mapping action=summary id=%s kva=%px bytes=%zu pages=%u virtual_contiguous=yes physical_contiguous=%s\n",
...
static void free_page_memory(void)
{
	if (exact_pages)
	{
		pr_info("KAPI_EVT domain=mm phase=cleanup action=free api=free_pages_exact id=exact_pages addr=%px bytes=%lu\n",
	...
	if (page_desc)
	{
		pr_info("KAPI_EVT domain=mm phase=cleanup action=free api=__free_pages id=page_desc addr=%px order=%u\n",
	...
	if (order_pages)
	{
		pr_info("KAPI_EVT domain=mm phase=cleanup action=free api=free_pages id=order_pages addr=%px order=%u\n",
	...
	if (zero_page)
	{
		pr_info("KAPI_EVT domain=mm phase=cleanup action=free api=free_page id=zero_page addr=%px order=0\n",
				(void *)zero_page);
		free_page(zero_page);
		zero_page = 0;
	}
	...
	if (one_page)
	{
		pr_info("KAPI_EVT domain=mm phase=cleanup action=free api=free_page id=one_page addr=%px order=0\n",
				(void *)one_page);
		free_page(one_page);
		one_page = 0;
	}
...
static int allocate_page_memory(void)
{
	...
	{
		pr_err("KAPI_EVT domain=mm phase=page_allocator action=error api=order_validation order=%u max_order=%u errno=%d\n",
			   BUDDY_PAGE_ORDER, MAX_PAGE_ORDER - 1, -EINVAL);
		return -EINVAL;
	}

	pr_info("KAPI_EVT domain=mm phase=page_allocator action=begin\n");
	...
	pr_info("KAPI_EVT domain=mm phase=page_allocator action=alloc api=__get_free_page id=one_page addr=%px bytes=%lu order=0 zeroed=no\n",
	...
	pr_info("KAPI_EVT domain=mm phase=page_allocator action=alloc api=get_zeroed_page id=zero_page addr=%px bytes=%lu order=0 zeroed=yes\n",
	...
	pr_info("KAPI_EVT domain=mm phase=page_allocator action=alloc api=__get_free_pages id=order_pages order=%u pages=%u addr=%px bytes=%zu zeroed=requested\n",
	...
	if (!page_desc_addr)
	{
		pr_err("KAPI_EVT domain=mm phase=page_allocator action=error api=page_address id=page_desc reason=no_direct_mapping errno=%d\n",
			   -ENOMEM);
		free_page_memory();
		return -ENOMEM;
	}
	...
	pr_info("KAPI_EVT domain=mm phase=page_allocator action=alloc api=alloc_pages id=page_desc order=%u struct_page=%px addr=%px bytes=%zu zeroed=requested\n",
	...
	pr_info("KAPI_EVT domain=mm phase=page_allocator action=alloc api=alloc_pages_exact id=exact_pages requested=%lu addr=%px pages=%u\n",
	...
	pr_info("KAPI_EVT domain=mm phase=page_allocator action=end allocations=5\n");
...
static void free_slab_memory(void)
{
	if (ctx)
	{
		pr_info("KAPI_EVT domain=mm phase=cleanup action=free api=kfree_sensitive id=slab_context addr=%px\n", ctx);
		kfree_sensitive(ctx);
		ctx = NULL;
	}
	...
	if (slab_buf)
	{
		pr_info("KAPI_EVT domain=mm phase=cleanup action=free api=kfree id=slab_buf addr=%px\n", slab_buf);
		kfree(slab_buf);
		slab_buf = NULL;
	}
}
...
static int allocate_slab_memory(void)
{
	...
	pr_info("KAPI_EVT domain=mm phase=slab action=begin\n");
	...
	pr_info("KAPI_EVT domain=mm phase=slab action=alloc api=kmalloc id=slab_buf requested=%u actual=%zu waste=%zu addr=%px zeroed=no\n",
			SLAB_BUFFER_SIZE,
			actual,
	...
	pr_info("KAPI_EVT domain=mm phase=slab action=alloc api=kzalloc id=slab_context requested=%zu actual=%zu waste=%zu addr=%px zeroed=yes\n",
			sizeof(*ctx),
			actual,
	...
	pr_info("KAPI_EVT domain=mm phase=slab action=end allocations=2\n");
...
static void free_custom_slab_cache(void)
{
	...
	for (i = 0; i < CUSTOM_CACHE_OBJECTS; i++)
	{
		if (custom_objects[i])
		{
			pr_info("KAPI_EVT domain=mm phase=cleanup action=free api=kmem_cache_free id=custom_object_%u addr=%px cache=%s\n",
	...
	if (custom_cache)
	{
		pr_info("KAPI_EVT domain=mm phase=cleanup action=destroy api=kmem_cache_destroy id=custom_cache name=%s\n",
				CUSTOM_CACHE_NAME);
...
static int create_custom_slab_cache(void)
{
	...
	pr_info("KAPI_EVT domain=mm phase=custom_cache action=begin\n");
	...
	if (!custom_cache)
	{
		pr_warn("KAPI_EVT domain=mm phase=custom_cache action=error api=kmem_cache_create errno=%d\n", -ENOMEM);
		return -ENOMEM;
	}

	pr_info("KAPI_EVT domain=mm phase=custom_cache action=create api=kmem_cache_create id=custom_cache name=%s object_size=%zu objects=%u\n",
	...
	for (i = 0; i < CUSTOM_CACHE_OBJECTS; i++)
	{
		...
		if (!custom_objects[i])
		{
			pr_warn("KAPI_EVT domain=mm phase=custom_cache action=error api=kmem_cache_alloc index=%u errno=%d\n", i, -ENOMEM);
		...
		pr_info("KAPI_EVT domain=mm phase=custom_cache action=alloc api=kmem_cache_alloc id=custom_object_%u addr=%px object_id=%u flags=0x%x name=%s\n",
				i,
				custom_objects[i],
				custom_objects[i]->id,
	...
	pr_info("KAPI_EVT domain=mm phase=custom_cache action=end objects=%u\n",
...
static void show_vmalloc_mapping(const char *name, const void *addr, size_t len)
{
	...
	for (i = 0; i < pages; i++)
	{
		...
		if (!page)
		{
			pr_warn("KAPI_EVT domain=mm phase=mapping action=missing_page id=%s index=%u kva=%px\n",
					name, i, this_kva);
			continue;
		}
		...
		pr_info("KAPI_EVT domain=mm phase=mapping action=page id=%s index=%u kva=%px struct_page=%px pa=%pa pfn=%lu\n",
...
static void free_vmalloc_memory(void)
{
	if (vzbuf)
	{
		pr_info("KAPI_EVT domain=mm phase=cleanup action=free api=vfree id=vzbuf addr=%px\n", vzbuf);
		vfree(vzbuf);
		vzbuf = NULL;
	}
	...
	if (vbuf)
	{
		pr_info("KAPI_EVT domain=mm phase=cleanup action=free api=vfree id=vbuf addr=%px\n", vbuf);
		vfree(vbuf);
		vbuf = NULL;
	}
}
...
{
	pr_info("KAPI_EVT domain=mm phase=vmalloc action=begin\n");

	vbuf = vmalloc(VMALLOC_BUFFER_SIZE);
	if (!vbuf)
	{
		pr_warn("KAPI_EVT domain=mm phase=vmalloc action=error api=vmalloc requested=%lu errno=%d\n",
				(unsigned long)VMALLOC_BUFFER_SIZE, -ENOMEM);
		return -ENOMEM;
	}

	memset(vbuf, 0x5a, VMALLOC_BUFFER_SIZE);
	pr_info("KAPI_EVT domain=mm phase=vmalloc action=alloc api=vmalloc id=vbuf requested=%lu addr=%px zeroed=no\n",
	...
	if (!vzbuf)
	{
		pr_warn("KAPI_EVT domain=mm phase=vmalloc action=error api=vzalloc requested=%lu errno=%d\n",
				(unsigned long)VMALLOC_BUFFER_SIZE, -ENOMEM);
		free_vmalloc_memory();
		return -ENOMEM;
	}

	pr_info("KAPI_EVT domain=mm phase=vmalloc action=alloc api=vzalloc id=vzbuf requested=%lu addr=%px zeroed=yes\n",
	...
	pr_info("KAPI_EVT domain=mm phase=vmalloc action=end allocations=2\n");
...
static int __init mm_kapi_init(void)
{
	...
	pr_info("KAPI_EVT domain=mm phase=lifecycle action=attached current_pid=%d current_cpu=%d current_comm=\"%s\"\n",
...
static void __exit mm_kapi_exit(void)
{
	pr_info("KAPI_EVT domain=mm phase=cleanup action=begin\n");
	print_addr("mm_kapi_exit", (unsigned long)mm_kapi_exit);
	free_vmalloc_memory();
	free_custom_slab_cache();
	free_slab_memory();
	free_page_memory();
	pr_info("KAPI_EVT domain=mm phase=cleanup action=end\n");
	pr_info("KAPI_EVT domain=mm phase=lifecycle action=detached\n");
}
...
static int __init kapi_init(void)
{
	...
	pr_info("KAPI_EVT domain=module phase=lifecycle action=run_begin schema=1 module=%s\n", KBUILD_MODNAME);

	ret = core_kapi_init();
	if (ret)
	{
		pr_err("KAPI_EVT domain=module phase=lifecycle action=run_end status=error stage=core errno=%d\n", ret);
		return ret;
	}
	...
	{
		core_kapi_exit();
		pr_err("KAPI_EVT domain=module phase=lifecycle action=run_end status=error stage=mm errno=%d\n", ret);
		return ret;
	}

	pr_info("KAPI_EVT domain=module phase=lifecycle action=ready status=ok\n");
	return 0;
}
...
static void __exit kapi_exit(void)
{
	mm_kapi_exit();
	core_kapi_exit();
	pr_info("KAPI_EVT domain=module phase=lifecycle action=run_end status=ok\n");
}
