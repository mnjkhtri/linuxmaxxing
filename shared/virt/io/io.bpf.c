 ...
 * Boundaries without a vector (entry, exit, userspace exit, plain kvm_apic writes) leave the vector unsampled so the UI shows NOT SAMPLED, never a made-up zero.
 *
...
int BPF_UPROBE(device_dma_transfer, void *dev, uint8_t *guest_mem_base, uint64_t guest_mem_size, uint32_t dir, uint64_t gpa, uint32_t len)
{
	...
	/* The DMA boundary has no interrupt vector, so IRR/ISR stay NOT SAMPLED and only DMA facts are recorded. */
