...
int main(int argc, char **argv)
{
	...
	mark_phase(marker_fd, "QEDU_PHASE sequence=1 phase=run kind=io action=begin pid=%d device=/dev/qedu\n", getpid());

	mark_phase(marker_fd, "QEDU_PHASE sequence=2 phase=sysfs kind=syscall action=begin pid=%d attribute=timeout_ms\n", getpid());
	write_interface(SYSFS_PATH, "1200\n");
	read_interface(SYSFS_PATH);
	mark_phase(marker_fd, "QEDU_PHASE sequence=3 phase=sysfs kind=syscall action=end pid=%d attribute=timeout_ms\n", getpid());

	mark_phase(marker_fd, "QEDU_PHASE sequence=4 phase=debugfs kind=syscall action=begin pid=%d file=status\n", getpid());
	read_interface(DEBUGFS_PATH);
	mark_phase(marker_fd, "QEDU_PHASE sequence=5 phase=debugfs kind=syscall action=end pid=%d file=status\n", getpid());

	/* Integer input selects the factorial engine and completes through one IRQ. */
	mark_phase(marker_fd, "QEDU_PHASE sequence=6 phase=factorial kind=mmio action=begin pid=%d input=10\n", getpid());
	...
	mark_phase(marker_fd, "QEDU_PHASE sequence=7 phase=factorial kind=mmio action=end pid=%d input=10 result=3628800 bytes=8\n", getpid());

	/* A 512-byte non-integer payload takes the RAM -> EDU -> RAM DMA path. */
	memset(dma_payload, 'x', sizeof(dma_payload));
	mark_phase(marker_fd, "QEDU_PHASE sequence=8 phase=dma_echo kind=dma action=begin pid=%d bytes=512\n", getpid());
	qedu_fd = open(QEDU_PATH, O_RDWR);
	...
	mark_phase(marker_fd, "QEDU_PHASE sequence=9 phase=dma_echo kind=dma action=end pid=%d bytes=512 read_calls=1\n", getpid());
	mark_phase(marker_fd, "QEDU_PHASE sequence=10 phase=run kind=io action=end pid=%d\n", getpid());
