 ...
 * Each callback serializes one post-enqueue tree state as a single JSON line for scheduler.html.
 *
 * Kernel pointers are printed as strings. They are identities used to connect nodes in the visualization, never addresses userspace should dereference.
 */
...
/*
 * libbpf invokes this after the kretprobe emits one event. Rejecting a short record protects the byte-for-byte ABI shared with struct context_event in cfs.bpf.c.
 */
static int handle_event(void *ctx, void *data, size_t len)
{
	...
	json_u64(&first, "timestamp_ns", event->timestamp_ns);
